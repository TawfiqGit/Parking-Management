package com.tawfiqdev.database.converts
...
class Converters {
    ...
    // StatusRez -> String
    @TypeConverter fun reservationStatusToString(value: StatusRez?): String? = value?.name
    @TypeConverter fun stringToReservationStatus(value: String?): StatusRez? = value?.let { StatusRez.valueOf(it) }
    ...
    @TypeConverter fun resStatusToString(v: ReservationStatus?) = v?.name
    @TypeConverter fun stringToResStatus(s: String?) = s?.let { ReservationStatus.valueOf(it) }

    @TypeConverter fun payStatusToString(v: PaymentStatus?) = v?.name
    @TypeConverter fun stringToPayStatus(s: String?) = s?.let { PaymentStatus.valueOf(it) }

    @TypeConverter fun spotTypeToString(v: SpotType?) = v?.name
    @TypeConverter fun stringToSpotType(s: String?) = s?.let { SpotType.valueOf(it) }
}